...
- Phase 7 — Reference Player / 高分玩家对比（聚合基线已落地：`RankingReference` 同副本同专精榜单分位 + `gapVsP50Pct`，Phase AB 修复字段投递；逐场对标已落地于 Phase AF：`AppService.compareToTopRun` + `CompareCard`；「逐决策参照高分玩家循环」的深度版仍规划中）